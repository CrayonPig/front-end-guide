...
// 后续构建时 rollup将__VERSION__替换成具体的值
Vue.version = '__VERSION__'

export default Vue
```

这里有 2 处关键的代码，`import Vue from './instance/index'` 和 `initGlobalAPI(Vue)`，初始化全局 Vue API（我们后续章节介绍），我们先来看第一部分，在 `src/core/instance/index.js`
...
到这里我们就找到了Vue的实际入口。Vue实质上就是一个用 Function 实现的 Class，然后它的原型 prototype 以及它本身都扩展了一系列的方法和属性。
